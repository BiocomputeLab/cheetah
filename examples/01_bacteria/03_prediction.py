...
seg_predict = ch.Segmenter(model_filename='./output/bacteria_model.json',
	                       weights_filename='./output/bacteria_model_weights.h5')

# Load a single image and corrisponding label set (ground truth)
single_image = ch.load_image('./data/test_image.tif', normalization='max')
single_label_set = ch.load_label_set('./data/test_image_labels.png', 3)
...
ch.plot_segmask(predicted_labels[0], y_true=single_label_set[0], class_to_plot=2, xtick_int=100,
...
ch.plot_predprob(predicted_labels[0], class_names=['cell border', 'cell body', 'background'], n_classes=3, xtick_int=100,
                  ytick_int=100, show_plt=False, save_imag=True,
